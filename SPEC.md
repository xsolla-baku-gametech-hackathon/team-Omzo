...
**Encode.** Tile the frame into an 8×8 grid of blocks, one payload bit per block — the globally unique 16-bit `watermarkId` plus an 8-bit CRC, so the 16 id bits get three copies each and the 8 checksum bits two. Subdivide each block into a 4×4 checkerboard of cells. A 1 bit brightens one diagonal of cells by `+2` luminance and darkens the other by `−2`; a 0 bit reverses it. Clamp. The frame's average brightness is unchanged — only the phase of the pattern carries the payload — and it is invisible at normal viewing.

**Decode.** For each block, take the mean luminance of each of its 16 cells and sum them with the checkerboard's alternating signs. Normalise by full amplitude and **clamp to ±1**. Sum the signed evidence across the copies of each bit, read the bits by sign, and verify the CRC. Return `{ watermarkId, confidence }` or `null`, where confidence is the mean absolute block evidence.

**Why the signal is read inside a block and not against neighbouring blocks.** An earlier draft of this spec said to compare each block's mean luminance against the median of its neighbours. That was measured and it does not work: on a game-like frame it produced **43% bit error**, a coin flip, because it asks a ±2 shift to be detected against the brightness of a _different part of the picture_, which varies by hundreds. It also had a payload-dependent hole — a run of identical bits makes every block agree with its surroundings and cancel, so `watermarkId` 1, the first id a platform ever issues, was unreadable while 1337 decoded perfectly. Reading cells against the cells beside them, a few dozen pixels away and almost always the same colour, makes the image subtract itself out. Measured across flat, gradient, noisy, near-black, blown-out and two game-like frames, at 1×, 2× and 3×, over 11 ids: **231/231 exact recoveries, 0 bit errors**.

The **clamp to ±1** is load-bearing. Without it one block lying across a hard edge — a white UI panel against a dark room — reads hundreds of units strong and outvotes the two clean copies of the same bit.

**Confidence floor 0.6**, and it sits in a measured gap rather than a guessed one: across those same frames a watermarked one never scored below 0.72 and an unwatermarked one never above 0.42. A CRC-8 alone lets 1 in 256 random payloads through, which is not a good enough filter for an accusation of leaking an unreleased build.

The decoder must tolerate **integer downscaling by 2× and 3×** so that an OS screenshot taken on a HiDPI display still decodes. This costs nothing: blocks and cells are defined as fractions of the image rather than as fixed pixel sizes, so a shrunken frame tiles into the same grid and the mean over a shrunken cell is the mean over the source pixels it came from. It is the reason the sub-cell grid is 4×4 and not finer — at 6×6 a 3× reduction blurs adjacent cells into each other and the signal cancels itself.
